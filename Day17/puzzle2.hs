...
corr =
  map ((\(x : y : _) -> (read x :: Int, read y :: Int)) . splitOn ",") $
    words
...
main = do
  ...
  let (((_ : x : _) : (x2 : _) : _) : ((_ : y : _) : (y2 : _) : _) : _) =
      ...
      veloc = [(x, y) | y <- [-1000 .. 1000], x <- [0 .. 1000], fire' (x, y)]
  print (length veloc)

fire (x, y) (x1, y1) (x2, y2) (dx, dy)
  | x1 <= x && x <= x2 && y1 <= y && y <= y2 = True
  | x2 < x || y < y1 = False
  | otherwise = fire (x + dx, y + dy) (x1, y1) (x2, y2) (drag dx, dy -1)
  where
    drag dx
      | dx < 0 = dx + 1
      | dx > 0 = dx -1
